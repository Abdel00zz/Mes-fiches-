...
export const BlockImageComponent: React.FC<BlockImageProps> = ({ img, onUpdate, onDelete }) => {
  const containerStyle: React.CSSProperties = img.position === 'float' ? { width: `${img.width}%` } : {};
  ...
  if (img.position === 'float') {
    ...
    containerClass += `flex w-full mb-4 justify-${img.align}`;
  }

  return (
    <div className={containerClass} style={containerStyle}>
      <div className="relative shadow-md rounded-md overflow-hidden bg-white hover:shadow-xl transition-shadow border border-slate-100" style={img.position !== 'float' ? { width: `${img.width}%` } : {}}>
